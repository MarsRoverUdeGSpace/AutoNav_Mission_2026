...
class ArucoDetectorNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('publish_annotated_compressed', True)   # recommended for HaLow
        ...
        self.overwrite_latest_per_id = bool(self.get_parameter('overwrite_latest_per_id').value)
        self.also_save_timestamped = bool(self.get_parameter('also_save_timestamped').value)
        ...
        if self.publish_annotated_compressed:
            self.pub_annotated_compressed = self.create_publisher(CompressedImage, self.annotated_topic, 1)

        # Optional raw annotated goes to a separate topic
        if self.publish_annotated_raw:
            self.pub_annotated_raw = self.create_publisher(Image, self.annotated_raw_topic, 1)

        # Subscriber (input image)
        if self.use_compressed:
            self.sub = self.create_subscription(
                CompressedImage, self.compressed_topic, self._on_compressed, self.image_qos
            )
            self.get_logger().info(f"Subscribed to CompressedImage: {self.compressed_topic}")
        else:
            self.sub = self.create_subscription(
        ...
        if self.pub_annotated_raw is not None:
            self.get_logger().warn(f"Publishing annotated RAW on: {self.annotated_raw_topic} (high bandwidth)")

    def _load_aruco_dict(self, dict_name: str):
        if not hasattr(cv2.aruco, dict_name):
            self.get_logger().warn(f"Invalid aruco_dictionary '{dict_name}', falling back to DICT_4X4_50")
    ...
    def _on_raw(self, msg: Image) -> None:
        try:
            ...
            return
        self._process_frame(frame, stamp_sec=self._stamp_to_sec(msg.header.stamp), header=msg.header)
    ...
    def _publish_annotated(self, annotated_bgr: np.ndarray, header) -> None:
        ...
        if self.pub_annotated_compressed is not None:
            try:
                ...
                ok, buf = cv2.imencode('.jpg', annotated_bgr, [int(cv2.IMWRITE_JPEG_QUALITY), int(q)])
                if not ok:
    ...
    def _crop_marker(self, bgr: np.ndarray, corner_set: np.ndarray, pad: int) -> Optional[np.ndarray]:
